...
export async function sourceNodes({boundActionCreators}, pluginOptions) {
	...
	const { queries, url: apiUrl } = await schema.validate(
		pluginOptions
	);
		...
		results.forEach((content, index) => {
			...
			rootElement.edges.forEach((childNode, edgeIndex) => {
			  ...
			  createNode({
			    ...rest,
			    ...node,
			    id: node.id || `__anilist__${rootName}__${edgeIndex}__`,
			    children: [],
			    parent: '__SOURCE__',
			    internal: {
			      type: `Anilist${upper(rootName)}`,
				...
				createNode({
				  ...rootElement,
				  id: rootElement.id || `__anilist__${rootName}__${index}`,
				  children: [],
				  parent: '__SOURCE__',
				  internal: {
				    type: `Anilist${upper(rootName)}`,
